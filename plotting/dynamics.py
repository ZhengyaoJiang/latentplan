...
import warnings
from plotting.fullobsenv import *
...
class MuJoCoRenderer:
    ...
    def render_plan(self, savepath, actions, observations_pred, state, fps=30):
        ...
        ## does not have an associated next_state in the sampled latentplan
...
def set_state(env, state):
    ...
    if state.size != qpos_dim + qvel_dim:
        raise ValueError(f'State size {state.size} does not match qpos_dim + qvel_dim {qpos_dim + qvel_dim}')

    env.set_state(state[:qpos_dim], state[qpos_dim:])

def overlay_frames(frames):
    composite = np.ones_like(frames[0]) * 255
    for frame in frames:
        mask = get_image_mask(frame)
        composite[mask] = frame[mask]
    return composite

def overlay_frames_with_alpha(frames):
    composite = np.ones_like(frames[0]) * 255
    length = len(frames)
    for i, frame in enumerate(frames):
        mask = get_image_mask(frame)
        progress = 1-(length-i)/length
        frame_alpha = np.ones_like(frame) * 255 * (1-progress) + frame * progress
        composite[mask] = frame_alpha[mask]
    return composite


def overlay_frames_with_alpha_old(frames, alpha_rate=0.9):
    imgs = []
    for i, frame in enumerate(frames):
        img = Image.fromarray(np.uint8(frame)).convert('RGB')
        alpha = np.uint8(255*alpha_rate**i)
        print(alpha)
        img.putalpha(alpha)
        imgs.append(img)
